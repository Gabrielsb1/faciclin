...
class FaciclinSupabaseUser extends BaseAuthUser {
  FaciclinSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> faciclinSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = FaciclinSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
